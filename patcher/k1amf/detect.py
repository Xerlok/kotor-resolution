...
def check_build(data, exe_path):
    ...
    if len(data) != BASE_SIZE:
        raise Refusal(
            ...
            "\n"
            "If you're on Steam: Steam ships a swkotor.exe that no mod can\n"
            "patch - not this one, not UniWS, not the high-res menus mod.\n"
            "Install the \"KOTOR Editable Executable\" first. It's a free\n"
            "download on DeadlyStream and it's the normal first step for\n"
            "this kind of mod. Then run UniWS and the high-res menus mod,\n"
            "then this.\n"
            "\n"
            "If you're on GOG: your own swkotor.exe should already work\n"
            "directly. This usually means a different patch/language build,\n"
            "or the old 4-CD retail disc, which this patcher doesn't\n"
            "recognise.\n"
            "\n"
