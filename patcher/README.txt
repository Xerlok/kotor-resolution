...
mess. This fixes it:
  ...
  * the area map in the menu fills the map screen instead of sitting small in one corner
  * your marker, your party and the map notes sit where they should
  * 250 map notes that were in the strange place are moved by hand
...
So you need three other things first, in this order.

  1. KOTOR Editable Executable - Steam only, skip if you're on GOG
     https://deadlystream.com/files/file/1320-kotor-editable-executable/

     Steam ships a swkotor.exe that's locked - no mod can edit it, including
     the two below. This is an unlocked copy you drop into your game folder.

     If you're on GOG, you don't need this. GOG's own swkotor.exe (v1.03) is
     already unlocked - it's the same file this unlocks, byte for byte, save
     for 16 bytes of unused header padding. Just start at step 2.

  2. UniWS
     https://www.wsgf.org/article/universal-widescreen-uniws-patcher

     Makes the game run on your screen resolution. Pick "Star Wars: KOTOR" from the
...
zip, it stops and tells you to unzip it.
...
Wait until it says it has finished, it may take some time to think.

If it says it can't find your game, drag your KOTOR folder (the whole main game folder, not just exe)
onto Install.bat and let go. That's the folder with swkotor.exe in it.

Windows may say "Windows protected your PC" with a blue box. That can happen to
every small mod tool - it is a normal windows reaction to the unknown code.
Click "More info", then "Run anyway".
If you'd rather not, see "Running it without the exe" at the bottom.
...
(Paste that into File Explorer's address bar to see it.) It is kept out of this
...
In case it can't find your game folder or you have two separate KOTOR versions installed,
You can also drag your game folder onto Uninstall.bat, the same way as in the installation.
...
"It says my menu files don't match"
    The k1hrm menus mod was run at one resolution and its menu files were copied
...
"Steam deleted my patched game"
    ...
    recognize. It'll do that to any exe mod. Keep a copy of your patched
    swkotor.exe somewhere outside the game folder.

"My antivirus doesn't like it"
    You can ignore it, windows just warns you if you try to run some code it doesn't know.
    See "Running it without the exe" below, if for some reason antivirus still won't
    let you proceed (for advanced users).
...
This route needs Python installed - Install.bat doesn't.
